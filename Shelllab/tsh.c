...
void eval(char *cmdline) 
{
	...
	sigaddset(&mask, SIGINT);  //SIGINT 시그널을 시그널 셋에 추가
	sigaddset(&mask, SIGTSTP); //SIGTSTP 시그널을 시그널 셋에 추가
...
int builtin_cmd(char **argv)
{
	...
	else if(!strcmp(cmd, "jobs")) { /* jobs command */
...
void sigchld_handler(int sig) 
{
	...
	while((pid=waitpid(-1, &child_status, WUNTRACED|WNOHANG)) >0) { //자식프로세스 종료를 대기
		if(WIFEXITED(child_status)) { //정상 종료 시
			deletejob(jobs, pid);
		}
		else if(WIFSIGNALED(child_status)) { //자식프로세스 종료를 체크
			printf("Job [%d] (%d) terminated by signal %d\n", pid2jid(pid), pid, WTERMSIG(child_status));
			deletejob(jobs, pid);//자식프로레스를 목록에서 제거
		}
		else if(WIFSTOPPED(child_status)) { //자식프로세스 정지를 체크
			printf("Job [%d] (%d) stopped by signal %d\n", pid2jid(pid), pid, WSTOPSIG(child_status));
			getjobpid(jobs, pid)->state = ST;//자식프로세스 상태 변경
		}
	}
	return;
}
